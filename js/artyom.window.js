...
var Artyom = (function () {
    ...
    function Artyom() {
        ...
        this.ArtyomVoicesIdentifiers = {
            ...
            // Dutch (holland)
            "nl-NL": ["Microsoft Frank - Dutch (Netherlands)", "nl-NL", "nl_NL"],
        ...
        if (window.hasOwnProperty('webkitSpeechRecognition')) {
            ...
            throw new Error("Artyom.js can't recognize voice without the Speech Recognition API.");
        }
    ...
    Artyom.prototype.pause = function(){
        if ('speechSynthesis' in window) {
            if(window.speechSynthesis.pending &&
                window.speechSynthesis.speaking &&
                ! window.speechSynthesis.paused){
                
                console.log("artyom pause", speechSynthesis)
                window.speechSynthesis.pause()
            }
            else if(window.speechSynthesis.pending &&
                window.speechSynthesis.speaking &&
                window.speechSynthesis.paused){
                console.log("artyom resume")
                window.speechSynthesis.resume()
            }
            else if(window.speechSynthesis.pending){
                console.log("artyom resume2")
                window.speechSynthesis.resume()
            }
        }
    }
    ...
    Artyom.prototype.initVoices = function( config ){
        return new Promise( ( resolve, reject ) => {
            function _getVoices(){
                return window.speechSynthesis.getVoices()
            }
            function filter( vcs, config ){
                var fnd = [];
                if( typeof config == 'object'){
                    if( fnd.length == 0){
                        for( var prop in config ){
                            fnd = vcs.filter( ( vc ) => {
                                return vc[prop].match(config[prop].replace("-", "_"));
                            })
                        }
                    }
                    if( fnd.length == 0)
                        for( var prop in config ){
                            fnd = vcs.filter( ( vc ) => { 
                                if( typeof vc[prop] == 'string' && prop == 'lang'){
                                    var parts = config[prop].split('-')
                                    return vc[prop].match( parts[0] )
                                }
                                else
                                    return vc[prop].match(config[prop])
                            })
                        }
                }
                if( fnd.length == 0 )
            ...
            if( typeof voices == 'undefined' || voices.length == 0){
                var timer = setInterval( () => {
                    ...
                    if( voices.length > 0){
                        clearInterval( timer )
                        resolve( config ? filter(voices, config) : voices )
                    }
                }, 250)
            }
            else 
    ...
    Artyom.prototype.initialize = function (config) {
        ...
        if (config.hasOwnProperty("voice")){
            _this.ArtyomProperties.voice = config.voice
        }
    ...
    Artyom.prototype.getActualVoice = function(){
        var q = this.ArtyomProperties.voice
        return q
    }
    ...
    Artyom.prototype.talk = function (text, actualChunk, totalChunks, callbacks) {
        ...
        var availableVoice = _this.ArtyomProperties.voice;
        if (callbacks) {
