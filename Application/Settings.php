...
class Settings{
            ...
            "edit.php" => array(
                __( "Attachments", EMPTY_PLUGIN_TEXT_DOMAIN ) => array(
                    "path_to_attachments" => array(
                        ...
                        'help_message' => __( 'Create a folder and assign write permission to it for system user : ', EMPTY_PLUGIN_TEXT_DOMAIN ) . get_current_user() . "."
                    )
                ),
                __( "Salesforce", EMPTY_PLUGIN_TEXT_DOMAIN ) => array(
    ...
    private $options_root = EMPTY_PLUGIN_TEXT_DOMAIN;
    ...
    public function __construct( $options, $page = null, $subUnderneath = null ){
        ...
        add_filter( 'plugin_action_links_' . EMPTY_PLUGIN_TEXT_DOMAIN . '/' . EMPTY_PLUGIN_TEXT_DOMAIN . '.php', array( $this, 'add_links_to_listed_plugin' ));
    ...
    */
    public function get_option( $root = null ){
        ...
        if( empty( $root ) ) $root = EMPTY_PLUGIN_TEXT_DOMAIN;

        $saved = get_option( $root );

        $defaults = $this->get_options_setting_defaults( EMPTY_PLUGIN_TEXT_DOMAIN );
        $ret = array();
    ...
    public function register_settings_page(){
        ...
        if( $this->user_has_rights() ){
            ...
            if( $this->underneath == null && $this->page == EMPTY_PLUGIN_TEXT_DOMAIN ){
                add_menu_page( 
                    __( 'Empty Plugin', EMPTY_PLUGIN_TEXT_DOMAIN ), 
                    __( 'Empty Plugin', EMPTY_PLUGIN_TEXT_DOMAIN ), 
                    'manage_options', 
                    $this->page,
                    $callable,
            ...
            // Posts admin menu )
                ...
                add_submenu_page(
                    $this->underneath,
                    __( 'Settings', EMPTY_PLUGIN_TEXT_DOMAIN ),
                    __( 'Settings', EMPTY_PLUGIN_TEXT_DOMAIN ),
                    'manage_options',
                    $this->page,
                    $callable
    ...
    public function sanitize( $input )
    {
        foreach( $input as $key => $value ){
            $optionArray = $this->get_option_from_options_settings( EMPTY_PLUGIN_TEXT_DOMAIN, $key );
            if( $optionArray ) {
        ...
        if( $this->user_has_rights() ){
            ...
            register_setting(
                ...
                array(
                    ...
                    'default' => $this->get_options_setting_defaults( EMPTY_PLUGIN_TEXT_DOMAIN )
                )
            );

            //$options_settings = $this->options_setting[ EMPTY_PLUGIN_TEXT_DOMAIN ];
            $sections = $this->get_all_sections_options_setting( EMPTY_PLUGIN_TEXT_DOMAIN );
            ...
            foreach( $sections as $section => $sectionValue){
                ...
                add_settings_section(
                    ...
                    $section . '_admin_' . EMPTY_PLUGIN_TEXT_DOMAIN 
                );  

                foreach( $sectionValue as $key => $value ){
                    add_settings_field(
                        EMPTY_PLUGIN_TEXT_DOMAIN . '_' . $key, 
                        $value[ 'label' ],  
    ...
    public function print_section_info(){
        //echo __( 'Settings : ', EMPTY_PLUGIN_TEXT_DOMAIN );
    }

    public function input_callback( ){

        $options = $this->get_all_sections_options_setting( EMPTY_PLUGIN_TEXT_DOMAIN );
    ...
    public function render_settings_page(){
        ...
        include( EMPTY_PLUGIN_DIR . "/templates/admin-settings.php" );
    }
    ...
    private function build_settings_link(){
        ...
        else if( $this->page && $this->page != EMPTY_PLUGIN_TEXT_DOMAIN )
        ...
        if( $this->page )
            ...
            $page = EMPTY_PLUGIN_POST_TYPE;

        $lnk = '<a href="' . get_admin_url() . $wp_page . '&page=' . $page . '">' . __( 'Settings', EMPTY_PLUGIN_TEXT_DOMAIN ) . '</a>';
